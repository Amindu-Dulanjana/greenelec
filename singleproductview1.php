...
$brandsid = Database::search("SELECT * FROM `product` LEFT JOIN `model_has_brand`
 ON product.model_has_brand_id = model_has_brand.id INNER JOIN images ON product.id = images.product_id  WHERE `brand_id` LIKE '" . $brand_d["id"] . "' LIMIT 4");
            ...
            <div class="col-12 bg-white">
                ...
                if($feed == 0 ){
                   ...
                   <div class="col-12 text-center">
                       <label class="form-label fs-3 fw-bold  text-black-50">There are no feedbacks to view.</label>
                   </div>
                ...
                for($a = 0; $a<$feed ; $a++) {
                    ...
                    $usermail = $feedrow["user_email"];

                    $user_rs = Database::search("SELECT * FROM `user` WHERE `email` = '".$usermail."' ;");
                    $user_d = $user_rs->fetch_assoc();
                ?>

                            <div class="col-12 col-lg-4 border border-1 border-danger rounded">
                                <div class="col-12">
                                    <span class="fs-5 fw-bold text-primary"><?php echo $user_d["fname"]." ".$user_d["lname"];?></span>
                                </div>
                                <div class="col-12">
                                    <span class="fs-5 fw-bold text-primary text-dark"><?php echo $feedrow["feed"]; ?></span>
                                </div>
                                <div class="col-12 text-end">
                                    <span class="fs-6 fw-bold text-primary text-black-50"><?php echo $feedrow["date"]; ?></span>
                                </div>
                            </div>
                <?php 
                }
